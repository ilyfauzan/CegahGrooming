...
async def predict(data: ChatInput):
    ...
    # 3. Jalankan deteksi Hybrid
    hasil = detect_grooming_hybrid(chat_history[session_id])
    
    print(f"Session: {session_id} | Input: {data.text} | Standalone: {hasil['standalone_prob']:.2f} | Context: {hasil['context_prob']:.2f} | Final: {hasil['label']}")
    ...
    return {
        ...
        "translated": translated_text, # Kirim terjemahan asli pesan ini

...
# Content to insert (Same content)
latar_belakang = [
    "1.1 Latar Belakang",
...
landasan_teori = [
    ...
    "Online grooming didefinisikan sebagai upaya sistematis yang dilakukan oleh pelaku dewasa untuk membangun hubungan emosional dan kepercayaan dengan anak atau remaja melalui platform digital (media sosial, aplikasi pesan, atau game online) with tujuan eksploitasi seksual. Proses ini bersifat progresif dan manipulatif, yang menurut Buitrago et al. (2022) sering kali melibatkan tahapan pengumpulan informasi personal, pembentukan rahasia bersama, hingga fase isolasi sosial terhadap korban.",
    "",
...
# Find a stable marker for BAB I
start_idx = find_paragraph_index(doc, "BAB I")
if start_idx == -1:
    start_idx = find_paragraph_index(doc, "BAB  I")

# Find marker for BAB II
...
if start_idx != -1 and end_idx != -1:
    ...
    # Insert new lines IN ORDER
    # Instead of calling on the same after_p (which causes reversal), 
    # we'll insert before its current child.
    # Actually, the most reliable way to insert in order is to iterate the list 
    # and always insert BEFORE after_p, which as I discovered, reverses.
    # So we SHOULD reverse the list if we use that logic, OR correctly identity the PREVIOUS insertion point.
    
    # Let's just NOT reverse the list and insert before after_p one by one?
    # No, that's what made it C, B, A.
    # Let's do this:
    insertion_point = after_p
    for line in full_new_chapter_1[::-1]: # Use slicing to reverse
        insertion_point = insertion_point.insert_paragraph_before(line)
        # Wait, if I insert before and update insertion_point to the NEW paragraph,
        # and then insert before THAT, I'm just going backwards from the end.
        # Actually, [::-1] ensures we start with the LAST line of the list and work up.
        # Line 10: insert before after_p -> [L10, after_p]
        # Line 9: insert before L10 -> [L9, L10, after_p]
        # This is correct.
        
    doc.save(doc_path)
    print("Fixed document order successfully.")
else:
